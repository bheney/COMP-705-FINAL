...
class TwelveData(APIWrapper):
    ...
    def get_price_history(self, symbol, interval, start_date, end_date):
        ...
        return self._json_to_tupple(time_series.as_json())
    ...
    def get_latest_price_history(self, symbol=None, interval=None, data_points=None):
        ...
        """
        # Validate parameters
        super().get_latest_price_history(symbol=symbol, interval=interval, data_points=data_points)
        if self._key is None:
            raise APIWrapper.APIException("No API key. TwelveData.set_api_key() not called.")
        if symbol is None:
            raise APIWrapper.APIException("No symbol provided")
        if interval is None:
            raise APIWrapper.APIException("No interval provided")
        if interval not in self.intervals:
            raise APIWrapper.APIException(f"Interval {interval} is not {self.intervals}")
        if data_points is not None and 0 > data_points > settings.TWELVE_DATA_MAX_DATA_SIZE:
            raise APIWrapper.APIException(
                f"Must request 0-{settings.TWELVE_DATA_MAX_DATA_SIZE} data points. You requested {data_points}.")
    ...
    def _json_to_tupple(self, json_data):
        prepared_data = []
